...
class elFinderVolumeMySQL extends elFinderVolumeDriver {
	...
	protected function _fopen($path, $mode='rb') {
        if (($res = $this->query('SELECT mtime, name FROM '.$this->tbf.' WHERE id="'.$path.'"'))
        && ($r = $res->fetch_assoc())) {
            /* EDITED */
            return @fopen($this->tmpPath . date('Y', $r['mtime']) .'/'. date('m', $r['mtime']) .'/'. sanitize_url($r['name']), 'r');
        }   
	}
	...
	protected function _unlink($path) {
        if (($res = $this->query('SELECT parent_id, name FROM '.$this->tbf.' WHERE id="'.$path.'"'))
        && ($r = $res->fetch_assoc())) {
            /* EDITED */
            @unlink($this->tmpPath.sanitize_url($r['name']));
        }          
	...
	protected function _save($fp, $dir, $name, $stat) {
		...
		if (! is_dir($this->tmpPath . $folder)) {
			if (! mkdir($this->tmpPath . $folder, 0755, TRUE)) {
				die('Error in mkdir');
			}
		}
		
        $path = $this->tmpPath . $folder .'/'. sanitize_url($name);
		...
		if (($tmpfile = tempnam($this->tmpPath, $this->id))) {
			if (($trgfp = fopen($tmpfile, 'wb')) == false) {
				...
				$sql = sprintf($sql, $this->tbf, $dir, $this->db->real_escape_string($name), '', $size, time(), $mime, $w, $h);
